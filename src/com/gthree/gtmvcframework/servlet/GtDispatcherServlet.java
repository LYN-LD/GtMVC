...
public class GtDispatcherServlet extends HttpServlet {
    ...
    private Handler getHandler(HttpServletRequest req) {
        ...
        requestURL = requestURL.replace(contextPath, "");//获取映射的路径
    ...
    private void loadConfig(String contextConfigLocation) {
        //先获取类加载器  在通过文件路径 使用类加载器获取文件的输入流
     ...
     * 扫描包路径
     *
     * @param scanPackage 包路径---->在配置文件中读取出来的
     *
     *  到这一步的时候配置文件已经加载完毕
     */
     ...
     * 初始化
     *
     * 此时包扫描完成  全限定类名已经被拼接存储完毕
     */
    private void startInstance() {
        if (classNames.isEmpty()) return;//未扫描，不执行
        try {
            //遍历全限定类名集合   获取其中的每一个全限定类名
            for (String className : classNames) {
                Class<?> classz = Class.forName(className);//根据类名反射获取字节码对象
    ...
    private void startHandlerMapping() {
        if (ioc.isEmpty()) return;
        for (Map.Entry<String, Object> entry : ioc.entrySet()) {
            Class<?> clazz = entry.getValue().getClass();//获取当前对象的类的字节码对象
